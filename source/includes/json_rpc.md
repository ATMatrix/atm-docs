...
To talk to an blockchain node from inside a JavaScript application use the [atn.js](https://github.com/ATNIO/atn-js) library, which gives a convenient interface for the RPC methods.
...
```bash
atnchain --rpc --rpccorsdomain "http://localhost:3000"
```

The JSON RPC can also be started from the [atnchain console](https://github.com/ATNIO/atn-console) using the `admin.startRPC(addr, port)` command.
...
Currently [chain-core](https://github.com/ATNIO/chain-core) provide JSON-RPC communication over http and IPC (unix socket Linux and OSX/named pipes on Windows). 
...
```
The atnchain console which comes with the web3 library can be used to make the calculation:
```js
